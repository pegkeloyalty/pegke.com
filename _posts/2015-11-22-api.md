...
<div class="twelve wide column">
  ...
  Two-legged OAuth, or "signed fetch", takes advantage of OAuth's signatures to authenticate server-to-server requests. It doesn't need to involve the user nor any access tokens.

  If a consumer site wants to make an authorized API call to a provider site, they can simply sign the request the same way that they would sign a request token request. The provider verifies the signature and services the request if the consumer is authorized.

  Two-legged OAuth is not well specified in any standards docs that I've seen. It's more of a convenient reuse of the signature scheme.
...
<div class="ui small message blue">
